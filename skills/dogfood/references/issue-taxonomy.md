...
1. **Visual scan** -- Examine the rendered DOM. Look for layout, alignment, and rendering issues.
2. **Interactive elements** -- Identify every button, link, and control in the DOM. Do they have proper handlers? Are there dead links?
3. **Forms** -- Find form elements. Check for validation attributes, required fields, and error handling.
4. **Navigation** -- Follow all navigation paths. Check breadcrumbs, back links, deep links.
5. **States** -- Check empty states, loading states, error states, and full/overflow states.
6. **Console / Errors** -- Look for error elements, failed request indicators, and warning messages in the DOM.
7. **Responsiveness** -- If relevant, check for responsive meta tags and viewport-dependent styling.
